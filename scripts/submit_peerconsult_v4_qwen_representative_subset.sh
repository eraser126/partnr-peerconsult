...
mkdir -p logs
array_job_raw="$(sbatch --parsable --export="ALL,PARTNR_REPRESENTATIVE_MANIFEST=${manifest}" --array="0-$((cards - 1))%${cards}" scripts/peerconsult_v4_qwen_representative_array.sbatch)"
array_job_id="${array_job_raw%%;*}"
merge_job_raw="$(sbatch --parsable --export=NONE --dependency="afterok:${array_job_id}" scripts/merge_peerconsult_v4_qwen_representative_subset.sbatch "$array_job_id" "$manifest")"
merge_job_id="${merge_job_raw%%;*}"

echo "Submitted ${episode_count}-episode representative PARTNR subset on ${cards} GPU shard(s): ${array_job_id}"
echo "Submitted CPU result merger: ${merge_job_id}"
echo "Monitor: squeue -j ${array_job_id},${merge_job_id}"
echo "Merged result: logs/peerconsult-v4-qwen-representative-${array_job_id}_merged/results/summary.json"
